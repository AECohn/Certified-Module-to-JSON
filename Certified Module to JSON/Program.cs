                ...
                if (!File.Exists(json))
                {
                    try
                    {
...

                        {
                            ...
                            Protocol = text.Substring(From, To - From ); //This seems stable, but, consider finding location of '}' before BSJB, and subtracting appropriate amount. Otherwise, consider using stacks (track {} and []) to evaluate when JSON ends
                        }
                        else
                        {
